...
async function requestCameraAccess() {
    // Проверяем, есть ли уже доступ к камере
    ...
    if (stream) {
        ...
        // Здесь можно добавить код для работы с видеопотоком
        stream.getTracks().forEach(track => track.stop()); // Останавливаем поток, если он не нужен
    } else {
