...
async function initCamera() {
    try {
        const stream = await navigator.mediaDevices.getUserMedia({ 
            video: { facingMode: { exact: "environment" } } // Используем основную камеру
        });
        video.srcObject = stream;
        video.play();
    } catch (error) {
        console.error("Ошибка инициализации камеры: ", error);
        alert("Не удалось инициализировать камеру. Пожалуйста, проверьте настройки разрешений.");
    }
}
...
async function startCamera() {
    try {
        ...
        // Убедитесь, что track установлен для управления фонариком
        [track] = stream.getVideoTracks();
        
        // Запускаем сканирование после успешного доступа к камере
        startScanning();
    } catch (error) {
...
function handleScanResult(data) {
    ...
    if (!uniqueValues.has(data)) {
        ...
        scannedData.push({ value: data, suffix: "" });
...
flashButton.addEventListener('click', async () => {
    ...
    try {
        ...
        alert("Ошибка управления фонариком. Пожалуйста, проверьте настройки устройства.");
    }
});
...
function startScanning() {
    ...
    codeReader.decodeFromVideoElement(video, (result, err) => {
        if (result) {
            console.log("Сканированный результат:", result.text); // Логируем результат
