...
let model;
let cameraAccessGranted = false; // Флаг для отслеживания доступа к камере
...
async function startCamera() {
    try {
        ...
        await requestCameraAccess(deviceId); // Передаем deviceId в функцию запроса доступа
    } catch (error) {
...
async function requestCameraAccess(deviceId) {
    if (cameraAccessGranted) return; // Если доступ уже предоставлен, выходим из функции
    ...
    try {
        ...
        cameraAccessGranted = true; // Устанавливаем флаг, что доступ предоставлен
    } catch (err) {
