...
async function initCamera() {
    if (!isWebRTCSupported()) {
        alert("Ваш браузер не поддерживает WebRTC. Пожалуйста, используйте более современный браузер.");
        return;
    }
    ...
    try {
        stream = await navigator.mediaDevices.getUserMedia({
            video: {
                facingMode: { exact: "environment" }, // Используйте только заднюю камеру
                width: { ideal: 1280 },
                height: { ideal: 720 }
            }
        });
        ...
        // Добавляем обработчик события для начала воспроизведения
        video.onloadedmetadata = () => {
            video.play();
            startScanning(); // Начинаем сканирование после загрузки видео
        };
    } catch (error) {
        console.error("Ошибка доступа к камере:", error);
        alert("Не удалось получить доступ к камере. Пожалуйста, проверьте разрешения или используйте другой браузер.");
    }
}
...
function handleCameraError() {
    alert("Камера недоступна. Вы можете загрузить изображение QR-кода для сканирования.");
    // Здесь можно добавить логику для загрузки изображения
}

// Вызов функции для запуска камеры
startCamera();
